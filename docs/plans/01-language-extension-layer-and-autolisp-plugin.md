...
- T10 scope (added 2026-09-25, plan 04 S6): the upstream artefact also carries the `lang-sniff` engine commits (plan 04 S1-S5: the sniff router, the `[match]` detect hook in `classify_file`, the augment kind). They touch `graphify/lang_registry.py`, `graphify/detect.py`, `graphify_lang/registry.py` and `graphify_lang/manifest.py` and contain no language-specific code. `graphify_lang/registry.py` and `manifest.py` are generic but sit outside `graphify/`, so `git diff v8...lang-registry -- graphify/` alone does not show them. The guard-tests regex above predates `tests/test_lang_sniff.py` and `tests/fixtures/sniff/`, which are new fork files, not edits to upstream tests.
