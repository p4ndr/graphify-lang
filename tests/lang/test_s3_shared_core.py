...
def test_s3_x1_recursive_calls_kept(tmp_path):
    ...
    (``obj.M`` with ``obj As`` its own class) dropped ``src == tgt``, and VBA
    read a procedure's own name as its result variable even in a Sub."""
    g = _build(tmp_path, {
    ...
    assert loops == {"fact", "Walk", "Fact", "Go"}     # `One = 1` is a result, not a call
