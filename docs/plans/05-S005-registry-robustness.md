...
- **M3 augment claim.** cc-kb's `[match]` claims every `.md`. Until the review-fix pass (cc-CR000.003 S5-M2) "an augment claims it" meant the augment adds anything to the built-in's result, and cc-kb adds a `cc_kb_links` attribute or a `code` ref to any page with a Markdown link or a backticked path: 349 of this repo's 436 tracked `.md` files, not only cc-kb documents, so a doc-only edit also ran `_rebuild_code`. It is now the augment's `watch` predicate (`watch_cc_kb`): any `docs/cc-*.md` by name alone (owner, review-fix S006: keeps hub -> spoke edges fresh for a new spoke), or a mention of another cc id, or a backticked path that is a file under the page's harness root. 46 of 436 are claimed (S5 rule without the name claim: 43); a page with only links stays a doc, so upstream `test_batch_modified_doc_only_does_not_rebuild` passes unchanged. Known limit: an edit that removes a non-`docs/cc-*` page's last cc mention and last in-scope code path does not rebuild; the stale or missing `cites` / `contains` edges stay until the next rebuild (the `needs_update` flag is still written). A deleted file counts by suffix (`[match]` or augment suffix), so the rebuild's reconcile drops it.
