...
| `05-S002-repo-and-ci-hygiene.md` | Plan 05 S002: repo and CI hygiene | Stage 2 of plan 05: the workflows parse, CI runs on the fork's release branches and tags, and the repo holds no junk. | ACTIVE |
| `05-S003-shared-plugin-core.md` | Plan 05 S003: shared plugin core | Stage 3 of plan 05: one shared module carries the id, sink and resolver contracts that the five plugins now copy, so H2 and M4 are fixed in… | ACTIVE |
| `05-S004-build-coherence.md` | Plan 05 S004: build coherence | Stage 4 of plan 05: an incremental build, a cached build and a clean build give the same graph. | ACTIVE |
| `05-S005-registry-robustness.md` | Plan 05 S005: registry robustness | Stage 5 of plan 05: one bad plugin cannot take down the others, plugin files trigger every rebuild path, and the registry has one clear loa… | ACTIVE |
