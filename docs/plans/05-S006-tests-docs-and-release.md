...
| Step | Action | Check |
|:-----|:-------|:------|
| S6.0 | Hub D5: `git fetch upstream`; rebase the `rr-s1`..`rr-s6` chain onto `upstream/v8` (rebase, never merge; resolve conflicts only in fork-owned code and registry-lookup lines). | Full pytest exits 0 with no upstream test file edited; `git diff upstream/v8...HEAD -- graphify/extractors/` is empty. Tree check (cc-CR000.003 S6-E1): `git range-diff <old-base>..<old-tip> <new-base>..<new-tip>` shows `!` only for conflicting commits; for each file in `git diff --name-only <old-base> <new-base>`, `git diff <old-base> <new-base> -- <file>` equals `git diff <old-tip> <new-tip> -- <file>` except the fork-owned `README.md`, `pyproject.toml`, `uv.lock`. |
...
| S6.1 | M12: `corpus` marker in `pyproject.toml`; the five corpus tests parametrized, each with a checked-in sample case that always runs and a private-repo case marked `corpus` (skip reason `corpus: <repo> not on this host`). bmake and ecschema reuse their fixture trees; `tests/lang/fixtures/corpus/` adds a synthetic CRLF `vba/ThisWorkbook.cls` and a synthetic ast-grep project; `tests/lang/conftest.py` gives `corpus_ls` (`git ls-files` or a glob). `pytest -m "not corpus and not perf" tests/ -q`: 6232 passed, 14 skipped, 6 deselected. | `842a67d` |
